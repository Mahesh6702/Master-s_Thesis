...
#include "MAX6675.h" //Temperature sensor library
...
enum mode{RUN, SLEEP, STOP, STANDBY};
enum peripheral{SPI, I2C, Ethernet};
...
extern int adrs_219; // Current sensor external variable

/* USER CODE END PTD */

/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define BROKER_IP		"192.168.0.20"
...
// Calling calibration function from INA219.c file
...
void MqttClientPubTask(void const *argument)
{
  ...
  while(1)
  {
	  mcu.Clockfrequency = HAL_RCC_GetSysClockFreq();
	  sprintf(str1,"System clock is %%luHz\r\n",mcu.Clockfrequency );
      md = RUN;
	  sprintf(str2,"%u",md);
